 ...
 * <\br>
 * 2023/07/24: add sync ability to update properties from mem to Nacos
 *
 * @author Eric Zhao, Ashes
 */
public class NacosDataSourceExt<T> extends AbstractDataSource<String, T> implements PropertiesSyncer<T> {
     ...
     */
    public NacosDataSourceExt(final String serverAddr, final String groupId, final String dataId,
                              Converter<String, T> parser) {
        this(NacosDataSourceExt.buildProperties(serverAddr), groupId, dataId, parser);
    }
     ...
     * @param parser     customized data parser, cannot be empty
     */
    public NacosDataSourceExt(final Properties properties, final String groupId, final String dataId,
                              Converter<String, T> parser) {
        super(parser);
        ...
        this.configListener = new Listener() {
            ...
            public void receiveConfigInfo(final String configInfo) {
                ...
                T newValue = NacosDataSourceExt.this.parser.convert(configInfo);
    ...
    private void customizeProperty() {
        try {
            Field property = AbstractDataSource.class.getDeclaredField("property");
            property.setAccessible(true);
            property.set(this, new NacosDynamicSentinelProperty<>(this));
    ...
    public boolean update(String rules) throws Exception {
        if (configService == null) {
            throw new IllegalStateException("Nacos config service has not been initialized or error occurred");
        }
        return configService.publishConfig(dataId, groupId, rules);
    }
    ...
    @Override
    public boolean sync(T t) {
        if (!(t instanceof List)) {
            return false;
        }
        try {
            update(JSONArray.toJSONString((List<T>) t));
            return true;
        } catch (Exception e) {
            e.printStackTrace();
        }
